<?php
/**
 * Stilero Social Promoter Facebook Group Plugin
 *
 * @version  1.0
 * @author Daniel Eliasson <daniel at stilero.com>
 * @copyright  (C) 2014-aug-12 Stilero Webdesign (http://www.stilero.com)
 * @category Plugins
 * @license	GPLv2
 */
...
class plgSocialpromoterStilerospfacebookgroup extends JPlugin {
    protected $Facebook;
    ...
    const SP_NAME = 'Facebook Group Plugin';
    const SP_DESCRIPTION = 'Posts links and photos to Facebook Groups';
    const SP_IMAGE = '';
    ...
    public function __construct(&$subject, $config = array()) {
        ...
        $language->load('plg_socialpromoter_stilerospfacebookgroup', JPATH_ADMINISTRATOR, 'en-GB', true);
        $language->load('plg_socialpromoter_stilerospfacebookgroup', JPATH_ADMINISTRATOR, null, true);
        $this->setParams();
    }
    ...
    protected function setParams(){
        ...
        $this->groupId = $this->params->def('fb_groups');
...
//        $response = $this->Facebook->Photos->publishFromUrl($url, $caption);
    ...
    public function getComments($photo_id){
        ...
        $processed = StileroSPFBOauthResponse::handle($response);
        $comments = array();
        if(!empty($processed->data)){
            foreach ($processed->data as $comment) {
                $commentClass = new stdClass();
                $timestamp = strtotime($comment->created_time);
                $commentClass->time = StileroSPFBBoardhelper::timeToText($timestamp);
                $commentClass->name = $comment->from->name;
                $commentClass->text = $comment->message;
